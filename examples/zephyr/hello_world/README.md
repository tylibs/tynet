...
Why? Because we can 🚀 —and because it demonstrates the basic usage of TyNet.  
...
static void onHelloWorld(const Event *aEvent) { LOG_INF("Action onHelloWorld() called from TyNet"); }

int main(void)
{
    // Subscribe to the TyNet using the state table. This registers the module
    // to receive and process events according to the defined rules.
    LOG_WRN("Subscribe module '%s' to TyNet", TAG);
    tbSubscribe(TB_SUBSCRIBER(TAG, stateTable, TB_TABLE_ROW_COUNT(stateTable), MAIN_STATE_START));

    while (true)
    {
        LOG_INF("Publish HELLO_WORLD event");
        // Publish the HELLO_WORLD event.  The TyNet will then look up this
        ...
        // is being sent with the event.    // publish an event to the TyNet
...
[00:00:00.000,000] <wrn> main: Subscribe module 'main' to TyNet
[00:00:00.000,000] <inf> main: Publish HELLO_WORLD event
[00:00:00.000,000] <inf> main: Action onHelloWorld() called from TyNet
[00:00:01.010,000] <inf> main: Publish HELLO_WORLD event
[00:00:01.010,000] <inf> main: Action onHelloWorld() called from TyNet
```
